...
function getMissingBadges(product: {
  name: string;
  storeName: string | null;
  logoName: string | null;
  ...
  if (!getDisplayName({ ...product, stockCode: null })) {
    badges.push("İsim eksik");
  }

  if (!product.imageUrl) {
    badges.push("Görsel yok");
  }

  if (!product.category) {
    badges.push("Kategori yok");
  }
...
function buildWhere(filters: ReturnType<typeof parseAdminProductFilters>) {
  ...
  if (filters.quality === "missing-image") {
    and.push({ OR: [{ imageUrl: null }, { imageUrl: "" }] });
  }

  if (filters.quality === "missing-category") {
    and.push({ OR: [{ category: null }, { category: "" }] });
  }

  if (filters.quality === "missing-name") {
    and.push({
      AND: [
        { OR: [{ name: "" }, { name: { equals: "" } }] },
        { OR: [{ storeName: null }, { storeName: "" }] },
        { OR: [{ logoName: null }, { logoName: "" }] },
      ],
    });
  }
...
export default async function AdminProductsPage({
  ...
  const [products, totalProducts, visibleProducts, hiddenProducts] =
    await Promise.all([
      prisma.product.findMany({
        where,
        orderBy: [{ showOnWebsite: "desc" }, { updatedAt: "desc" }],
        skip,
        take: filters.pageSize,
      }),
      prisma.product.count({ where }),
      prisma.product.count({ where: { showOnWebsite: true } }),
      prisma.product.count({ where: { showOnWebsite: false } }),
    ]);
      ...
      <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div className="rounded-[1.5rem] border border-amber-200 bg-amber-50 p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-amber-800">
            Import sonrası kontrol bekleyen ürünler.
          </p>
        </div>
      </section>

      <section className="rounded-[1.75rem] border border-[#17201c]/10 bg-white p-4 shadow-sm sm:p-6">
        <form className="grid grid-cols-1 gap-3 lg:grid-cols-[1fr_180px_200px_130px]">
          <input
            name="q"
            ...
            <table className="min-w-full divide-y divide-[#17201c]/10 text-sm">
              ...
              <tbody className="divide-y divide-[#17201c]/10">
                {products.map((product) => {
                  ...
                  const missingBadges = getMissingBadges(product);
                  ...
                  return (
                    <tr key={product.id} className="align-top">
                      ...
                      <td className="min-w-56 px-3 py-4 text-[#68746e]">
                        <p>Logo: {product.logoIsActive ? "Aktif" : "Pasif"}</p>
